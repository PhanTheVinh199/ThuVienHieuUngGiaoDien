<template>
  <div class="container">
    ...
    <div v-for="effect in allScrollEffects" :key="effect.id" :ref="setEffectRef(effect)" style="margin-bottom: 60px;">
...
export default {
  ...
  computed: {
    allScrollEffects() {
      const group = effectsData.find((g) => g.category === 'scroll');
      return group ? group.effects : [];
    },
    types() {
      const typesSet = new Set(this.allScrollEffects.map((e) => e.type));
